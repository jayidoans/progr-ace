...
export default async function TrainingPage({ searchParams }: { searchParams: Promise<{ error?: string; detail?: string }> }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {data.isAuthor ? <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200"><h2 className="text-xl font-bold">Import XLSX</h2><p className="mt-2 text-sm text-gray-600">Upload, validate, and preview before creating a draft. Confirmation is atomic and idempotent.</p><div className="mt-6"><TrainingImportForm raceGoals={data.raceGoals} /></div></section> : null}
    </div>
  );
}
