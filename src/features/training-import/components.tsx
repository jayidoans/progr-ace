...
export function TrainingImportForm({ raceGoals }: { raceGoals: ProgramRaceGoal[] }) {
  return (
    <form action={uploadTrainingTemplate} className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-medium text-gray-800 sm:col-span-2">Official ProgrACE XLSX<input accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className={input} name="template" required type="file" /><span className="mt-1 block text-xs text-gray-500">Maximum 1 MB. XLSX only. The file is parsed but not retained.</span></label>
...
export function ImportPreview({ previewId, plan, warnings, importedProgramId }: { previewId: string; plan: NormalizedTrainingPlan; warnings: ImportIssue[]; importedProgramId: string | null }) {
  return (
    <div className="space-y-6">
      ...
      {importedProgramId ? <a className="inline-flex rounded-md bg-indigo-600 px-4 py-2 font-semibold text-white" href={`/dashboard/training/${importedProgramId}`}>Open imported program</a> : <form action={confirmTrainingImport}><input name="previewId" type="hidden" value={previewId} /><button className="rounded-md bg-emerald-600 px-4 py-2 font-semibold text-white hover:bg-emerald-500" type="submit">Confirm atomic draft import</button></form>}
    </div>
  );
}
